...
//   // Send message to parent window about widget state
//   useEffect(() => {
//     try {
//       setIsInIframe(window.self !== window.top);
      
//       if (isInIframe) {
//         // Notify parent about current state (icon only vs full chat)
//         const widgetState = isChatOpen ? 'open' : 'icon';
//         window.parent.postMessage({
//           type: 'CHATBOT_STATE',
//           state: widgetState,
...
//       console.error("Error checking iframe status:", e);
//     }
//   }, [isInIframe, isChatOpen, isMaximized]);
...
//     const handleClickOutside = (event: MouseEvent) => {
//       // If chat is open and click is outside the chat container
...
//     // Add listener to parent window if possible
//     try {
//       window.parent.document.addEventListener('mousedown', handleClickOutside);
      
//       return () => {
//         window.parent.document.removeEventListener('mousedown', handleClickOutside);
//       };
//     } catch (e) {
//       console.error("Cannot access parent document:", e);
//       // Fallback to listening in current window
//       document.addEventListener('mousedown', handleClickOutside);
...
//   const toggleIcons = () => {
//     setShowIcons((prev) => !prev);
//     // If we're showing icons, notify parent to ensure proper size
//     if (isInIframe && !showIcons) {
//       window.parent.postMessage({
//         type: 'CHATBOT_STATE',
//         state: 'icons',
//         maximized: false
...
//     setIsMaximized(true);
//     // Notify parent window to maximize iframe
//     if (isInIframe) {
//       window.parent.postMessage({
//         type: 'CHATBOT_STATE',
//         state: 'open',
...
//   const handleRestore = () => {
//     setIsMaximized(false);
//     // Notify parent window to restore iframe
//     if (isInIframe) {
//       window.parent.postMessage({
//         type: 'CHATBOT_STATE',
//         state: 'open',
...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    window.parent.postMessage({
      type: 'widgetState',
      state: state
    }, "*"); // Consider replacing "*" with your specific domain for security
        ...
        <div ref={widgetRef}>
          {showIcons && (
            <div className="fixed bottom-2 z-50 right-4 w-[95%] max-w-[500px] mx-auto flex items-center justify-between bg-white rounded-[28px] shadow-lg p-2 animate-slideInRight">
              <div className="flex items-center space-x-1 sm:space-x-2">
                {[
                  { src: "/images/finalbot.gif", alt: "Chatbot", onClick: toggleChat, tooltip: "Chat with Punjab Govt. AI Assistant" },
                  { src: "/images/awhatsapp.gif", alt: "WhatsApp", onClick: handleWhatsAppClick, tooltip: "WhatsApp Chatbot" },
                  { src: "/images/acall.gif", alt: "Contact", onClick: handlePhoneClick, tooltip: "State Helpline 1100" },
                  { src: "/images/aqr.gif", alt: "QR Scan", onClick: handleQRClick, tooltip: "QR Code to open Whatsapp Chatbot" },
                ].map((item, index) => (
                  <div key={index} className="animate-iconAppear flex-shrink-0" style={{ animationDelay: `${index * 0.1}s` }}>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <div className="w-12 h-12 flex items-center justify-center rounded-full shadow-md bg-white hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                          <Image
