...
useEffect(() => {
  ...
  if (isInIframe) {
    ...
    setTimeout(() => sendMessageToParent(currentState), 10);
  }
}, [isInIframe, showIcons, isChatOpen, isMaximized, showQRImage]);
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
 ...
 const toggleChat = () => {
  ...
  if (newChatState) {
    ...
    setIsMaximized(false);
    setShowQRImage(false);
    setTimeout(() => sendMessageToParent('chat'), 10); 
    sendInitialMessages();
  } else {
  
    if (showIcons) {
      setTimeout(() => sendMessageToParent('icons'), 10);
    } else {
      setTimeout(() => sendMessageToParent('icon'), 10);
    }
  }
};
  const handleLanguageChange = async (selectedLanguage: string) => {
...
const handleMaximize = () => {
  setIsMaximized(true);
  setTimeout(() => sendMessageToParent('maximized'), 10);
};
...
const handleQRClick = () => {
  ...
  setShowQRImage(true);
  setIsMaximized(false);
  setShowIcons(false); // Hide icons when showing QR
  
  // Then send message
  setTimeout(() => sendMessageToParent('qr'), 10);
};
...
const handleCloseQR = () => {
  ...
  // Then send message to show icons
  setTimeout(() => sendMessageToParent('icon'), 10);
};
